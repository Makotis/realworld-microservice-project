...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t makotis/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker makotis/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push makotis/frontendservice:latest "
                    }
                }
            }
        }
         ...
         stage('ZAP Dynamic Testing | DAST') {
             steps {
                 sshagent(['OWASP-Zap-Credential']) {
                     sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.144.48.113 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://3.134.244.35/" || true'
    ...
    always {
        ...
        slackSend channel: '#om--multi-microservices-alerts', //update and provide your channel name
